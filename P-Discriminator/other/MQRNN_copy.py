...
class MQRNN(object):
    ...
    def predict(self, train_target_df, train_covariate_df, test_covariate_df, columns):
        train_target_tensor = torch.tensor(train_target_df[columns].to_numpy())
        train_covariate_df = train_covariate_df.to_numpy()
        train_covariate_df = torch.tensor(train_covariate_df)
        ...
        train_target_tensor = train_target_tensor.to(self.device)
        train_covariate_df = train_covariate_df.to(self.device)
        next_covariate_tensor = next_covariate_tensor.to(self.device)

        with torch.no_grad():
            input_target_covariate_tensor = torch.cat([train_target_tensor, train_covariate_df], dim=1)
