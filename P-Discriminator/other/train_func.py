...
from data_process import MQRNN_dataset
...
def train_fn(encoder: Encoder, gdecoder: GlobalDecoder, ldecoder: LocalDecoder, dataset: MQRNN_dataset, lr: float,
             batch_size: int, num_epochs: int, device):
    """
        MQRNN模型训练函数
    """
    ...
    for epoch in range(num_epochs):
        ...
        for (cur_series_tensor, cur_covariate_tensor, cur_real_vals_tensor) in data_iter:
            ...
            loss = calc_loss(cur_series_tensor, cur_covariate_tensor, cur_real_vals_tensor, encoder, gdecoder, ldecoder, device)
